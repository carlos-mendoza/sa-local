...
$all = get_ancestors( current($terms)->term_id, 'policies-type' );
$all[] = current($terms)->term_id;
$cp_id = get_the_ID();
	...
	<div id="primary" class="content-area page_sidebar">
		<div id="content" class="site-content wrapper " role="main">
            <div class="right_policies">
                 <?php sa_breadcrumbs(); ?>
                  <?php 
                //$su = array_filter(get_field('user_type'));
                    $su = get_post_meta(get_the_ID(), 'user_type', true);
                    if(!$su){
                        $su = array();
                    }
                    if(in_array($user_role,$su) || $user_role == "administrator"){
                	while ( have_posts() ) : the_post(); ?>
                ...
                <?php }else{  ?>
                  <div class="entry-content cf policiy_content">
                    <h2 ><?php echo get_option( 'access_denied_message' ); ?></h2>
                </div>
                <?php } ?>
            </div>
