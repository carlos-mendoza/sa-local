 ...
 *
 */
class SA_Subpages_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'widget_subpages', 'description' => __( 'Lists current section subpages', 'sa-subpages' ) );
		parent::__construct( 'sa-subpages-widget', __( 'Theme Subpages Widget', 'be-subpages' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( !is_singular( $post_types ) && !apply_filters( 'sa_subpages_widget_display_override', false ) )
			return;
		...
		$parents[] = $post->ID;
		$parents = apply_filters( 'sa_subpages_widget_parents', $parents );
		...
		$depth = 1;
		$subpages = get_pages( apply_filters( 'sa_subpages_widget_args', $args, $depth ) );
		...
		global $sa_subpages_is_first;
		$sa_subpages_is_first = true;
		...
		if( 1 == $instance['title_from_parent'] ) {
			...
			if( 1 == $instance['title_link'] )
				$title = '<a href="' . get_permalink( $parents[0] ) . '">' . apply_filters( 'sa_subpages_widget_title', $title ) . '</a>';
		}	
		...
		if( !isset( $instance['deep_subpages'] ) )
			$instance['deep_subpages'] = 1;
			
		if( !isset( $instance['nest_subpages'] ) )
			$instance['nest_subpages'] = 1;

		// Print the tree
		$this->sa_build_subpages( $subpages, $parents, $instance['deep_subpages'], $depth, $instance['nest_subpages'] );
	 ...
	 *
	 */
	function sa_build_subpages( $subpages, $parents, $deep_subpages = 1, $depth = 1, $nest_subpages = 1 ) {
		if( empty( $subpages ) )
			return;
			
		global $post, $sa_subpages_is_first;
		...
		foreach ( $subpages as $subpage ) {
			...
			// First menu item
			if( $sa_subpages_is_first )
				$class[] .= 'first-menu-item';
			$sa_subpages_is_first = false;
			
			$class = apply_filters( 'sa_subpages_widget_class', $class, $subpage );
			$class = !empty( $class ) ? ' class="' . implode( ' ', $class ) . '"' : '';

			echo '<li' . $class . '><a href="' . get_permalink( $subpage->ID ) . '">' . apply_filters( 'sa_subpages_page_title', $subpage->post_title, $subpage ) . '</a>';
			// If nesting supress the closing li
			if (!$nest_subpages)
				echo '</li>';
			
			do_action( 'sa_subpages_widget_menu_extra', $subpage, $class );
			...
			if ( $deep_subpages && in_array( $subpage->ID, $parents ) ) {
				...
				$deeper_pages = get_pages( apply_filters( 'sa_subpages_widget_args', $args, $depth ) );
				$depth++;
				$this->sa_build_subpages( $deeper_pages, $parents, $deep_subpages, $depth, $nest_subpages );
			}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'title_from_parent' => 0, 'title_link' => 0, 'deep_subpages' => 1, 'nest_subpages' => 1 );
